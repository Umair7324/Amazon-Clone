...
userRouter.post("/api/add-to-cart",auth,async(req,res)=>{
    try{
        ...
        if(user.cart.length==0){
           await user.cart.push({product,quantity:1});
        }else{
            ...
            if(isProductFound){
                let productt=await user.cart.find((producttt)=>producttt.product._id.equals(product._id));
                productt.quantity+=1;
            }else{
...
userRouter.post("/api/order",auth,async(req,res)=>{
    try{
        ...
        user.cart=[];
      user=  await user.save();
      let order=new Order({products,totalPrice,address,userId:req.user,orderAt: new Date().getTime(),});
